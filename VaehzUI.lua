...
local KEYBOARD_IMAGE = "rbxassetid://121332782788896"
	...
	function api:CreateButton(bcfg)
		...
		function api:CreateToggle(tocfg)
			...
			local function normalizeKeyCode(value)
				if typeof(value) == "EnumItem" and value.EnumType == Enum.KeyCode and value ~= Enum.KeyCode.Unknown then
					return value
				end
				if type(value) == "string" and value ~= "" then
					local ok, result = pcall(function() return Enum.KeyCode[value] end)
					if ok and result and result ~= Enum.KeyCode.Unknown then return result end
				end
				return nil
			end
			local function formatKeyCode(value)
				if not value then return "_" end
				local aliases = {
				...
				return aliases[value.Name] or string.upper(value.Name)
			end

			local keyCode = normalizeKeyCode(tocfg.Keybind or tocfg.DefaultKeybind or tocfg.Bind)
			...
			create("TextLabel", {
				...
				Position = UDim2.new(0, 10, 0.5, 0), Size = UDim2.new(1, -154, 1, 0), Parent = btnEl,
			})
			...
			-- Keybind control: the keyboard slides left while waiting for a key.
			local keybindZone = create("Frame", {
				Name = "Keybind", BackgroundTransparency = 1, AnchorPoint = Vector2.new(1, 0.5),
				Position = UDim2.new(1, -56, 0.5, 0), Size = UDim2.fromOffset(84, 26),
				ZIndex = 3, Parent = row,
			})
			local keybindBtn = create("TextButton", {
				Text = "", AutoButtonColor = false, BackgroundColor3 = Theme.Secondary,
				BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0),
				ZIndex = 3, Parent = keybindZone,
			})
			corner(keybindBtn, 5)
			local keyImage = create("ImageLabel", {
				Name = "KeyboardIcon", Image = tocfg.KeybindImage or KEYBOARD_IMAGE,
				BackgroundTransparency = 1, ImageColor3 = Theme.SubText,
				AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, 2, 0.5, 0),
				Size = UDim2.fromOffset(19, 19), ZIndex = 4, Parent = keybindBtn,
			})
			local captureLabel = create("TextLabel", {
				BackgroundTransparency = 1, Text = "...", FontFace = FONT_TITLE,
				TextColor3 = Theme.Accent, TextSize = 11, TextTransparency = 1,
				TextXAlignment = Enum.TextXAlignment.Left, AnchorPoint = Vector2.new(0, 0.5),
				Position = UDim2.new(0, 25, 0.5, 0), Size = UDim2.fromOffset(24, 18),
				ZIndex = 4, Parent = keybindBtn,
			})
			local keyLabel = create("TextLabel", {
				BackgroundTransparency = 1, Text = formatKeyCode(keyCode), FontFace = FONT_TITLE,
				TextColor3 = keyCode and Theme.Accent or Theme.SubText, TextSize = 11,
				TextXAlignment = Enum.TextXAlignment.Right, AnchorPoint = Vector2.new(1, 0.5),
				Position = UDim2.new(1, -3, 0.5, 0), Size = UDim2.fromOffset(48, 18),
				TextTruncate = Enum.TextTruncate.AtEnd, ZIndex = 4, Parent = keybindBtn,
			})

			local api_obj = {}
			local function refreshKeybindVisual()
				keyLabel.Text = formatKeyCode(keyCode)
				keyLabel.TextColor3 = keyCode and Theme.Accent or Theme.SubText
			end
			local function stopCapture(selectedKey)
				if captureConnection then
					captureConnection:Disconnect()
					captureConnection = nil
				end
				listening = false
				if selectedKey ~= false then keyCode = selectedKey end
				refreshKeybindVisual()
				captureLabel.TextTransparency = 1
				tween(keyImage, TI, { Position = UDim2.new(0, 2, 0.5, 0), ImageColor3 = Theme.SubText })
				tween(keybindBtn, TI, { BackgroundTransparency = 1 })
				if tocfg.KeybindCallback and selectedKey ~= false then
					task.spawn(tocfg.KeybindCallback, keyCode)
				end
			end
			local function beginCapture()
				...
				keyLabel.Text = ""
				captureLabel.Text = "..."
				captureLabel.TextTransparency = 0
				tween(keyImage, TI, { Position = UDim2.new(0, -4, 0.5, 0), ImageColor3 = Theme.Accent })
				tween(keybindBtn, TI, { BackgroundTransparency = 0.35 })
			...
			function api_obj:SetKeybind(v)
				keyCode = normalizeKeyCode(v)
				refreshKeybindVisual()
				if tocfg.KeybindCallback then task.spawn(tocfg.KeybindCallback, keyCode) end
			end
			function api_obj:GetKeybind() return keyCode end
			function api_obj:ClearKeybind() api_obj:SetKeybind(nil) end

			local inputConnection = UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
				if gameProcessedEvent or listening or not keyCode then return end
				if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == keyCode then
					api_obj:Set(not state)
				end
			end)
			...
			keybindBtn.MouseEnter:Connect(function()
				if not listening then tween(keybindBtn, TI, { BackgroundTransparency = 0.5 }) end
			end)
			keybindBtn.MouseLeave:Connect(function()
				if not listening then tween(keybindBtn, TI, { BackgroundTransparency = 1 }) end
			end)
			keybindBtn.Activated:Connect(beginCapture)
